...
GitLab CI/CD Visual Editor is a single-page application (SPA) built with React that runs entirely in the browser. It integrates with GitLab's API for validation and template fetching but works fully offline for core editing functionality.
